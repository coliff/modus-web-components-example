...
let ModusSelect = class {
  ...
  render() {
    ...
    return (index.h("div", { role: "listbox", "aria-disabled": this.disabled, "aria-label": this.ariaLabel, "aria-required": this.required }, this.label || this.required
      ? index.h("div", { class: 'label-container' }, this.label ? index.h("label", null, this.label) : null, this.required ? index.h("span", { class: "required" }, "*") : null)
      : null, index.h("div", { class: inputContainerClass }, index.h("button", { class: buttonClass, disabled: this.disabled, onClick: () => this.handleButtonClick(), type: "button", "aria-invalid": !!this.errorText }, index.h("div", { class: "dropdown-text" }, this.value ? this.value[this.optionsDisplayProp] : null), index.h(IconTriangleDown, { size: '12' })), index.h("div", { class: dropdownListClass }, this.options.map((option, index$1) => index.h("div", { "aria-selected": index$1 === this.activeItemIndex, "aria-label": option[this.optionsDisplayProp], class: `dropdown-list-item ${index$1 === this.activeItemIndex ? 'active' : ''}`, key: createGuid(), onClick: () => this.handleItemSelect(option), onMouseEnter: () => this.activeItemIndex = index$1 }, option[this.optionsDisplayProp])))), this.errorText ? index.h("label", { class: "sub-text error" }, this.errorText) :
      this.validText ? index.h("label", { class: "sub-text valid" }, this.validText) :
        this.helperText ? index.h("label", { class: "sub-text helper" }, this.helperText) :
          null));
  }
