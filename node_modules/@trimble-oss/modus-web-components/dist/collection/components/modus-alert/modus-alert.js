...
export class ModusAlert {
  ...
  render() {
    ...
    const iconSize = '24';
    return (h("div", { "aria-label": this.ariaLabel, class: className, role: "alert", tabIndex: 0 },
      this.type === 'error' ? h(IconError, { size: iconSize }) : null,
      this.infoTypes.includes(this.type) ? h(IconInfo, { size: iconSize }) : null,
      this.type === 'success' ? h(IconCheckCircle, { size: iconSize }) : null,
      this.type === 'warning' ? h(IconWarning, { size: iconSize }) : null,
      h("div", { class: "message" }, this.message),
      this.dismissible ? h(IconClose, { size: "18", onClick: () => this.dismissClick.emit() }) : null));
  }
